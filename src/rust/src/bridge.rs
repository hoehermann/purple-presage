...
pub fn append_message(message: Message) {
    ...
    let get_cstring_ptr = |s: Option<std::ffi::CString>| (s.map_or(std::ptr::null(), |s| s.as_ptr()));
    ...
    let groups: Vec<CGroup> = message
        ...
        .map(|g| {
            let mut c_group = CGroup {
                key: std::ffi::CString::new(hex::encode(g.key.clone())).ok(),
    ...
    let c_groups: Vec<crate::bridge_structs::Group> = message
        .groups
        .iter()
        .zip(groups)
        .map(|(rg, cg)| crate::bridge_structs::Group {
    ...
    let c_message = crate::bridge_structs::Message {
        ...
        blob: message.attachment.map_or(std::ptr::null(), |a| a.as_ptr() as *const std::os::raw::c_void),
